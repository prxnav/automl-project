...
A `run_instructions.md` file that guides through the command to run the designed AutoML solution on the training set of the _final-test-dataset_.
...
A second command that given (i), (ii), or (iii) would do the needful that yields predictions for `test_X` for the _final-test-dataset_. This is the `final_test_preds.npy`.

#### **4) Team information**

Upload a file `team_info.txt` with the list of matriculation IDs of team members (_NO NAMES_). (E.g.: 1234567, 7654321)
...
- [ ] Team info
- [x] _Example to denote task being done_
<!-- This is a comment. -->
